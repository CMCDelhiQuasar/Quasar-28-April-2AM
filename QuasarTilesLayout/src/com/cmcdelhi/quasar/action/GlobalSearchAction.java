...
public class GlobalSearchAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			queryResultStudentList.addAll(ss.getStudent(null, qvalue, null, 0,
					null, 0, null, 0));
			// can be contact no
			queryResultStudentList.addAll(ss.getStudent(null, 0, null, 0, null,
					qvalue, null, 0));
		...
		try {
			// can be payment Id
			long qvalue = Long.parseLong(queryString);
			queryResultPaymentList.addAll(ps.getPayment(0, qvalue, null, 0, 0,
					null, null, 0, 0, null, 0, 0, 0, 0, null));
			// / DD No
			queryResultPaymentList.addAll(ps.getPayment(0, 0, null, 0, 0, null,
					null, qvalue, 0, null, 0, 0, 0, 0, null));
			// Card Number
			queryResultPaymentList.addAll(ps.getPayment(0, 0, null, 0, 0, null,
					null, 0, 0, null, 0, 0, 0, qvalue, null));

			// Reciept Number
			queryResultPaymentList.addAll(ps.getPayment(0, 0, null, 0, 0, null,
					null, 0, 0, null, 0, qvalue, 0, 0, null));
		...
		try {
			...
			double qvalueDouble = Double.parseDouble(queryString);
			queryResultPaymentList.addAll(ps.getPayment(0, 0, null, 0,
					qvalueDouble, null, null, 0, 0, null, 0, 0, 0, 0, null));

			// can be Deposited Amount Amount
			queryResultPaymentList.addAll(ps.getPayment(0, 0, null,
					qvalueDouble, 0, null, null, 0, 0, null, 0, 0, 0, 0, null));
